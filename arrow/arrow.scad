...
if (part == "whole") {
  ...
  translate([-90, 25, 0] * scale1)
    connector(thickness, scale1);
  translate([-70, 25, 0] * scale1)
    connector(thickness, scale1);
  translate([0, 0, 0] * scale1)
    arrowPart1(thickness, scale1, 0, 400);
  translate([140, -400, 0] * scale1)
    arrowPart2(thickness, scale1, 400, 300);
  translate([-190, -700, 0] * scale1)
    arrowPart3(thickness, scale1, 700, 300);
} else {
...
module connector(thickness, scale1) {
  translate([0, 0, thickness / 4 - epsi])
    cube([8 * scale1, 50 * scale1, thickness / 2], center = true);
}

module arrowPart1(thickness, scale1, partFrom, partLength) {
  difference() {
    arrowPart(thickness, scale1, partFrom, partLength);
    translate([0, (partFrom + partLength) * scale1, 0])
      connector(thickness, scale1);
  }
}
...
module arrowPart(thickness, scale1, partFrom, partLength) {
  intersection() {
    ...
    translate([0, (partFrom + partLength / 2) * scale1, 0])
      cube(partLength * scale1, center = true);
  }
}
