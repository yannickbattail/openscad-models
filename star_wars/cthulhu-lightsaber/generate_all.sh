...
if command -v notify-send >/dev/null 2>&1; then
  if [ $status -eq 0 ]; then
    notify-send -u normal "cthulhu-lightsaber" "Generation finished successfully."
  else
    notify-send -u critical "cthulhu-lightsaber" "Generation FAILED with exit code $status."
  fi
else
