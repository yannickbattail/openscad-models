...
module geodesic_sphere(r = -1, d = -1, $fn = $fn) {
        ...
        [
          for (f = faces)
            let (p0 = p[f[0]], p1 = p[f[1]], p2 = p[f[2]])
              ...
              for (outp = [p0, p1, p2, midpt(p0, p1), midpt(p1, p2), midpt(p0, p2)])
                outp
        ],
          ...
          for (i = [0:len(faces) - 1])
            let (base = 6 * i) // points generated in multiples of 6
              for (outf = [
                ...
                outf // "identity" for-loop saves having to flatten
        ]
      ];

  // recursive wrapper for subdivpf that subdivides "levels" times
  function multi_subdiv_pf(pf, levels) =
    levels == 0 ? pf : multi_subdiv_pf(subdivpf(pf), levels - 1);
