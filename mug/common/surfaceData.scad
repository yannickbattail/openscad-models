...
function surfaceDataf(M, center = false) =
  let (n = len(M), m = len(M[0]), miz = min([
    for (Mi = M)
      min(Mi)
  ]), minz = miz < 0 ? miz - 1 : -1, ctr = center ? [-(m - 1) / 2, -(n - 1) / 2, 0] : [0, 0, 0], points = [ // original data points
    for (i = [0:n - 1])
      for (j = [0:m - 1])
        [j, i, M[i][j]] + ctr,
    [0, 0, minz] + ctr,
    ...
    for (i = [0:n - 1])
      for (j = [0:m - 1])
        let (med = i == n - 1 || j == m - 1 ? 0 : (M[i][j] + M[i + 1][j] + M[i + 1][j + 1] + M[i][j + 1]) / 4)
          [j + 0.5, i + 0.5, med] + ctr
...
module surfaceData(M, center = false, convexity = 10) {
  ...
  polyhedron(a.points, a.faces, convexity = convexity);
}
